...
PH_CLASSES: list[dict[str, Any]] = [
    {"label": "Extremely Acidic", "min": None, "max": 4.5, "color": "#B91C1C", "range": "pH < 4.5"},
    {"label": "Strongly Acidic", "min": 4.5, "max": 5.5, "color": "#EF4444", "range": "pH 4.5 – 5.4"},
    {"label": "Moderately Acidic", "min": 5.5, "max": 6.5, "color": "#F59E0B", "range": "pH 5.5 – 6.4"},
    {"label": "Neutral / Near Neutral", "min": 6.5, "max": 7.3, "color": "#84CC16", "range": "pH 6.5 – 7.2"},
    {"label": "Slightly Alkaline", "min": 7.3, "max": 7.9, "color": "#10B981", "range": "pH 7.3 – 7.8"},
...
# SAFE envelope detector: only features EXPLICITLY tagged as extents/bounding
# boxes are removed — valid rectangular grid datasets (climate fishnets,
# concession blocks) survive because they are not tagged.
...
def _frame_like(props: dict[str, Any]) -> bool:
    if props.get("is_extent") is True or props.get("layer_type") == "bbox" or "BBOX" in props:
        return True
...
def transform_swalim_feature(feature: dict[str, Any], source_layer: str) -> dict[str, Any] | None:
    ...
    if _frame_like(props) or not _is_thematic_polygon(feature.get("geometry")):
        return None
...
async def get_swalim_soil_ph(
    ...
    if not force_refresh:
        payload = disk_read("swalim_soil_ph", cache_ttl_s)
        if payload is not None:
            _CACHE[_CACHE_KEY] = (time.monotonic(), payload, "disk-cache")
    ...
    async with _LOCK:
        ...
        disk_write("swalim_soil_ph", data)
