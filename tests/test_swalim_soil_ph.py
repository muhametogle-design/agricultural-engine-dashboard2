...
def _fresh_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dc, "CACHE_DIR", tmp_path)  # sandbox the disk cache per test
...
@pytest.mark.parametrize("value,label,color", [
    (3.9, "Extremely Acidic", "#B91C1C"),
    (4.4, "Extremely Acidic", "#B91C1C"),
    (4.5, "Strongly Acidic", "#EF4444"),
    (5.4, "Strongly Acidic", "#EF4444"),
    (5.5, "Moderately Acidic", "#F59E0B"),
    (6.4, "Moderately Acidic", "#F59E0B"),
    (6.5, "Neutral / Near Neutral", "#84CC16"),
    (7.2, "Neutral / Near Neutral", "#84CC16"),
    (7.3, "Slightly Alkaline", "#10B981"),
...
def test_transform_uses_official_attributes():
    ...
    assert p["PH_CLASS"] == "Neutral / Near Neutral"
...
def test_safe_bbox_detector_metadata_only():
    # SAFE semantics: an untagged perfect rectangle (a valid climate fishnet /
    # concession grid cell) is PRESERVED.
    grid_cell = {
        "type": "Feature",
        "geometry": {"type": "Polygon", "coordinates": [[[40.9, -1.66], [41.0, -1.66], [41.0, -1.56], [40.9, -1.56], [40.9, -1.66]]]},
        "properties": {**WFS_FEATURE["properties"], "GRID_ID": "FISHNET-0042"},
    }
    assert swalim_soil.transform_swalim_feature(grid_cell, "afgooye_soil") is not None
    # ...but explicitly tagged envelopes are removed.
    for flag in ({"is_extent": True}, {"layer_type": "bbox"}, {"BBOX": "40.9,-1.66,51.45,12.05"}):
        tagged = {**WFS_FEATURE, "properties": {**WFS_FEATURE["properties"], **flag}}
        assert swalim_soil.transform_swalim_feature(tagged, "afgooye_soil") is None
